...
export const router = <T extends Record<string, OperationType<any, any>>>(
  ...
  const route: any = new Hono<{ Bindings: Bindings; Variables: any }>().onError(
    (err, c) => {
  ...
  Object.entries(obj).forEach(([key, operation]) => {
    ...
    if (operation.type === "query") {
      if (operation.schema) {
        route.get(path, queryParsingMiddleware, ...operationMiddlewares, ((
          c: Context,
        ) => {
          const ctx = c.get("__middleware_output") || {};
        ...
        route.get(path, ...operationMiddlewares, ((c: Context) => {
          const ctx = c.get("__middleware_output") || {};
      ...
      if (operation.schema) {
        route.post(path, bodyParsingMiddleware, ...operationMiddlewares, ((
          c: Context,
        ) => {
          const ctx = c.get("__middleware_output") || {};
        ...
        route.post(path, ...operationMiddlewares, ((c: Context) => {
          const ctx = c.get("__middleware_output") || {};
